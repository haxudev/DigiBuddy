 ...
 *
 * `depth` is how many directories deep the script sits inside the skill, and
 * `parents[depth]` is therefore the skill's own root. Note that `parents[1]` is
 * the grandparent, not the parent -- writing `.parent` here for a script one
 * directory down would point at `scripts/_lib`, and the skill would fail with
 * `ModuleNotFoundError` the first time the agent ran it.
 */
function entrypointShim(entry: ManifestEntrypoint, depth: number): string {
  return [
    ...
    "import sys",
    "",
    `sys.path.insert(0, str(Path(__file__).resolve().parents[${depth}] / "_lib"))`,
    "",
...
/**
 * Importable packages the archive holds that no discovered skill would receive.
 *
 * A repository keeps its library in one place and its skills in another, which
 * is right for a repository and wrong for a bundle: each skill is extracted
 * alone, so anything outside its own directory is simply absent. Only a
 * manifest can say which directory is the library, so when there is none this
 * reports what would be dropped.
 */
function strandedCode(archive: Archive, root: string, skills: string[]): string[] {
  const prefix = root ? `${root}/` : "";
  const found = new Set<string>();
  for (const path of archive.entries.keys()) {
    if (!path.startsWith(prefix)) continue;
    const relative = path.slice(prefix.length);
    if (!relative.endsWith("/__init__.py")) continue;
    // Inside a skill it already travels; the question is only about code that
    // sits outside every one of them.
    if (skills.some((skill) => relative.startsWith(`${skill}/`))) continue;
    const directory = relative.slice(0, -"/__init__.py".length);
    // The package root, not every sub-package it happens to contain.
    if ([...found].some((seen) => directory.startsWith(`${seen}/`))) continue;
    found.add(directory);
  }
...
export function explodeBundle(payload: Buffer, fileName: string): ExplodedBundle {
  ...
  if (relative.has(BUNDLE_MANIFEST)) {
    ...
    const stranded = strandedCode(archive, root, discovered);
    if (stranded.length > 0) {
      // Discovery can find a SKILL.md; it cannot guess that `src/thing/` is the
      // package those skills import. Without a manifest that code stays in the
      // archive, and the skill installs looking complete and then fails the
      // first time the agent runs anything in it. Say so before it is deployed,
      // because afterwards the symptom is a ModuleNotFoundError in a sandbox.
      notes.push(
        `Shared code was left behind: ${stranded.join(", ")}. A skill bundle is ` +
          `extracted on its own, so nothing outside it travels with it. Declare ` +
          `it under "shared" in ${BUNDLE_MANIFEST} to vendor it into each skill.`,
      );
    }
  }
