...
Hold the microphone button next to **+** to record a voice message. Releasing it converts the browser recording to PCM WAV, asks `mai-transcribe-1.5` to detect the language and transcribe it, and sends the result as a separate turn. `Esc` or a cancelled pointer gesture discards the recording; the 60-second limit stops and sends automatically. Voice input preserves the text draft, attachments, and armed skills already in the composer. Microphone capture requires HTTPS, except on `localhost`.

The browser sends audio only to the signed-in same-origin `/api/transcribe` route. Configure `MAI_TRANSCRIBE_ENDPOINT` with the root URL of a Speech in Foundry Tools resource, for example `https://name.cognitiveservices.azure.com`. The route uses Speech API `2025-10-15`, omits `locales` for multilingual detection, and requests `mai-transcribe-1.5` enhanced mode. Grant the WebUI managed identity **Cognitive Services User** on that Speech resource; it obtains a token for `https://cognitiveservices.azure.com/.default`. Audio is kept in memory for this synchronous request and is not written to the configuration store.
